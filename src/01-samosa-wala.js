  ...
  return {
    ...
    getPopularItem() {
      ...
      this.sales.forEach(sale => {
        totalQuantitiy[sale.item] = (totalQuantitiy[sale.item] || 0) + sale.quantity;
      });

      let maxItem = null;
      let maxQty = -1;

      for (let item in totalQuantitiy) {
        if (totalQuantitiy[item] > maxQty) {
          maxQty = totalQuantitiy[item];
          maxItem = item;
        }
      }
